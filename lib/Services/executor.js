...
class Executor extends Request {
   ...
   * @param {Object} payload
   */
  bulkUpdate(applicationId, userId, payload) {
    const path = `/rules/bulk`;
    const query = this.setTenant(applicationId, userId);
    return this.send('PUT', path, { payload, query });
  }
   ...
   * @param {string} ruleId
   */
  lockRule(applicationId, userId, ruleId) {
    const path = `/rules/${ruleId}/lock`;
    const query = this.setTenant(applicationId, userId);
   ...
   */
  getRulesByThings(applicationId, userId, thingIds) {
    const path = `/v1.0/rules/things`;
    const query = this.setTenant(applicationId, userId);

    // Manually add array parameters with bracket notation if needed
    // Most backends accept both formats, but if bracket notation is required:
    // thingIds.forEach((id, index) => {
    //   query[`ids[${index}]`] = id;
    // });

    // Standard repeated parameter format (ids=id1&ids=id2&ids=id3)
    query.ids = thingIds;
