class Cayenne {
   ...
   * @param {String} device_id The device id to get attributes for
   * @param {String} user_id The user id of the user that owns the parent device
   * @param {Object} query
   */
  getAttributeGroups(device_id, user_id, query) {
    const url = `${this.url}/things/${device_id}/groups`;
   ...
   * @param {String} device_id The id of the parent device
   * @param {String?} group_id Optional: Updates group, if null creates instead
   ...
   * @param {String?} payload.device_id Optional: The device_id this group is attached to. Overriden by device_id paramter
   * @param {UUID} payload.user_id The user_id of the device owner
   */
  createOrUpdateGroup(device_id, group_id, payload) {
    var url = null;
    if (group_id == null) {
      url = `${this.url}/things/${device_id}/groups`;
    } else {
      url = `${this.url}/things/${device_id}/groups/${group_id}`;
    }
   ...
   */
  deleteGroup(device_id, group_id) {
    const url = `/things/${device_id}/groups/${group_id}`;
   ...
   * @param {String?} attribute_id Optional: Updates attribute, if null creates instead
   * @param {Object} payload The attribute to create or update
   * @param {UUID?} payload.id Optional: The attribute id, will be overriden by attribute_id parameter
   * @param {String} payload.name The name of the attribute
   * @param {Integer} payload.order The display order of the attribute
   * @param {String?} payload.device_id Optional: The device_id this attribute is attached to. Overriden by device_id paramter
   * @param {UUID} payload.user_id The user_id of the device owner
   * @param {UUID?} payload.group_id The group_id of the parent group
   ...
   */
  createOrUpdateAttribute(device_id, group_id, attribute_id, payload) {
    var url = null;
    if (attribute_id == null) {
      url = `${this.url}/things/${device_id}/groups/${group_id}/attributes`;
    } else {
      url = `${
        this.url
      }/things/${device_id}/groups/${group_id}/attributes/${attribute_id}`;
    }
    return this.auth.send(this.service, 'PUT', url, { payload });
  }
   ...
   */
  deleteAttribute(device_id, group_id, attribute_id) {
    const url = `/things/${device_id}/groups/${group_id}/attributes/${attribute_id}`;
    return this.auth.send(this.service, 'DELETE', url);
  }
