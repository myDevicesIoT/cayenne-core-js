class Cayenne {
   ...
   * @param {Object} query
   */
  countThingDataTypesV2(query) {
    const url = `${this.url}/v2/things/datatypes/count`;
    return this.auth.send(this.service, 'GET', url, { query });
  }
   ...
   * @param {Object} query
   */
  countThingDataTypePropertiesV2(query) {
    const url = `${this.url}/v2/things/datatypes/${typeId}/properties`;
    return this.auth.send(this.service, 'GET', url, { query });
  }
  ...
  createThingDataTypePropertyV2(typeId, payload){
    ///v2/things/datatypes/{id}/properties/{pid}
  ...
  updateDeviceTypeChannel(deviceTypeId, channelId, payload) {
    const url = `${this.url}/things/types/${deviceTypeId}/channels/${channelId}`;
    return this.auth.send(this.service, 'PUT', url, { payload });
  }
