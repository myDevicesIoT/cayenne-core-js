class Cayenne {
   ...
   * @param {typeof LRU} Cache
   */
  constructor(url, Auth, Cache) {
  ...
  getRegistryDevice(hardwareId) {
    ...
    const registryDevice = this.auth.send(this.service, 'GET', url);
    this.cache.del(`${registryDevice.device_type_id}:device_type`);
  ...
  getDeviceTypeMetaByKey(deviceTypeId, key) {
    const cacheKey = `${deviceTypeId}:${key}`;
    let cachedValue = this.cache.get(cacheKey);
    ...
    try {
      value = this.auth.send(this.service, 'GET', url);
    } catch (error) {
      if (error.status !== 404) {
        throw error;
      }
