...
class Core {
  ...
  constructor(clientId, clientSecret, authorizationUrl, options) {
    options = Object.assign({ logger: true, isOffline: true }, options);

    this.auth = new Auth(clientId, clientSecret, authorizationUrl, options);

    // 30 minutes LRU, need to wait 30 minutes when switching device_type_meta
    // this is seldom happen
    this.cache = new LRU({
      max: 500,
    ...
    this.cayenne = new Cayenne(options.cayenneBaseUrl, this.auth, this.cache);
