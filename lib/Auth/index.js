  ...
  async send(service, method, url, opts, retry = 0) {
    ...
    try {
      ...
      const response = await superagent(method, url)
        .set(opts.headers)
        .timeout({
          response: opts.timeout || 60000,
          deadline: opts.deadline || 90000
        })
        .query(opts.query)
        .send(opts.payload);

      const interval = this.timeDelta(begin);

      // if response time larger than 1s, error
      if (interval > 1000) {
        this.log(
          `${service} ${method}, code: ${response.status}, t: ${interval} ms -> url: ${url}, query: ${JSON.stringify(opts.query || {})}`, 'error'
        );
      } else {
        this.log(
          `${service} ${method}, code: ${response.status}, t: ${interval} ms -> url: ${url}, query: ${JSON.stringify(opts.query || {})}`
        );
      }
