...
class CoreError extends Boom {
  ...
  constructor(message, options) {
    ...
    // Preserve original message for error propagation
    // this.message is overwritten to 'Internal error' above
    // Boom's reformat() hides output.payload.message for 5xx
    ...
    // Restore real message in output.payload for server errors
    if (this.originalMessage && this.isServer) {
      this.output.payload.message = this.originalMessage;
    }
